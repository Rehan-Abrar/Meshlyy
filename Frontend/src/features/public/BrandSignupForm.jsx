...
const AUDIENCE_LOCATIONS = ['Global | USA', 'North America', 'Europe', 'Asia', 'South America'];
...
const BrandSignupForm = () => {
  ...
  const [form, setForm] = useState({
    fullName: '', workEmail: '', phone: '', password: '', confirmPassword: '', city: '', location: '',
      ...
      <form className={styles.formContainer} onSubmit={handleSubmit}>
        ...
        <div className={styles.sectionCard}>
          ...
          <div className={styles.grid2}>
            ...
            <Field id="city" label="City">
              <input type="text" id="city" name="city" className={styles.input} placeholder="New York" value={form.city} onChange={handleChange} />
            </Field>
          </div>
          <div className={`${styles.fieldGroup} ${styles.mt-4}`}>
            <label className={styles.fieldLabel}>Location</label>
            <input type="text" name="location" className={styles.input} placeholder="State/Province, Country" value={form.location} onChange={handleChange} />
          </div>
        </div>
        ...
        <div className={styles.sectionCard}>
          ...
          <div className={styles.grid2}>
            ...
            <Field id="businessType" label="Business Type">
              <select id="businessType" name="businessType" className={styles.select} value={form.businessType} onChange={handleChange}>
            ...
            <Field id="businessSize" label="Business Size">
              <select id="businessSize" name="businessSize" className={styles.select} value={form.businessSize} onChange={handleChange}>
        ...
        <div className={styles.sectionCard}>
          ...
          <div className={`${styles.grid2} ${styles.mt-4}`}>
            <Field id="campaignType" label="Campaign Type">
              <select id="campaignType" name="campaignType" className={styles.select} value={form.campaignType} onChange={handleChange}>
                <option value="">Select Type...</option>
                {CAMPAIGN_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </Field>
            <Field id="budget" label="Monthly Budget Range">
              <select id="budget" name="budget" className={styles.select} value={form.budget} onChange={handleChange}>
                <option value="">Select Budget Range...</option>
                {BUDGET_RANGES.map(b => <option key={b} value={b}>{b}</option>)}
              </select>
            </Field>
            <Field id="creatorPref" label="Creator Type Preference">
              <select id="creatorPref" name="creatorPref" className={styles.select} value={form.creatorPref} onChange={handleChange}>
                <option value="">Select Tier...</option>
                {CREATOR_TIERS.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </Field>
        ...
        <div className={styles.sectionCard}>
          ...
          <div className={styles.grid2}>
            ...
            <Field id="audienceLocation" label="Audience Location">
              <select id="audienceLocation" name="audienceLocation" className={styles.select} value={form.audienceLocation} onChange={handleChange}>
                {AUDIENCE_LOCATIONS.map(l => <option key={l} value={l}>{l}</option>)}
              </select>
            </Field>
            <Field id="targetAge" label="Target Audience Age">
              <select id="targetAge" name="targetAge" className={styles.select} value={form.targetAge} onChange={handleChange}>
            ...
            <Field id="language" label="Language">
              <input type="text" id="language" name="language" className={styles.input} placeholder="English, Spanish, etc." value={form.language} onChange={handleChange} />
            </Field>
          </div>
          <div className={`${styles.fieldGroup} ${styles.mt-4}`}>
        ...
        <div className={styles.sectionCard}>
          ...
          <div className={`${styles.fieldGroup} ${styles.mt-4}`}>
            ...
            <input type="text" name="taxId" className={styles.input} placeholder="EIN / VAT Number" value={form.taxId} onChange={handleChange} />
          </div>
        </div>
        ...
        <div className={styles.sectionCard}>
          ...
          <div className={styles.grid2}>
            ...
            <Field id="preferredContact" label="Preferred Contact Method">
              <select id="preferredContact" name="preferredContact" className={styles.select} value={form.preferredContact} onChange={handleChange}>
